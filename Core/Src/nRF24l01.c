...
void nRF24Init() {
	...
	nRF24WriteReg(RF_SETUP, 0b1110); // power = -16dBm, data rate = 2Mbps
 ...
 * @param size: The number of bytes of data to transmit.
 *
 * @returns: 0 if transmission was OK, -1 if transmission failed.
 */
int nRF24Transmit(uint8_t* data, uint8_t size) {
  HAL_GPIO_WritePin(NRF24_CS_PORT, NRF24_CS_PIN, GPIO_PIN_RESET);
